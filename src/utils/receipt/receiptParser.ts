...
import { detectCategory } from "./categoryKeywords";
...
export function parseReceiptText(text: string) {
  ...
  const merchantLine = lines.find(
    (l: string) =>
      l.length > 3 &&
      !l.match(/total|amount|tax|gst|cgst|sgst|rs|₹/i)
  );

  const merchantName = merchantLine
    ? { value: merchantLine, confidence: "MEDIUM" as Confidence }
    : { value: null, confidence: "LOW" as Confidence };
  ...
  const dateMatch =
    ...
    cleaned.match(/\b\d{2}[\/\-]\d{2}[\/\-]\d{2}\b/);

  const date = dateMatch
    ? { value: dateMatch[0], confidence: "MEDIUM" as Confidence }
    : { value: null, confidence: "LOW" as Confidence };
